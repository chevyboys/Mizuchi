...
Module
  ...
  .addCommand({
    ...
    hidden: true,
    permissions: (msg) => (msg.author.id === Module.config.ownerId) || msg.member?.roles.cache.some(r => [
      ...
      Module.config.snowflakes.roles.BotMaster].includes(r.id)),
    process: async (msg) => {
  ...
  .addCommand({
    ...
    hidden: true,
    permissions: (msg) => (msg.author.id === Module.config.ownerId) || msg.member.roles.cache.has(Module.config.snowflakes.roles.BotMaster) || msg.member.roles.cache.has(Module.config.snowflakes.roles.BotAssistant),
  ...
  .addEvent("ready", () => {
    ...
    Module.client.guilds.cache.get(Module.config.snowflakes.guilds.PrimaryServer).members.fetch({ time: 300000 }).catch(e => u.errorHandler(e, "Error fetching members on ready event in Bot Admin module", Module.config.identifier));
    u.setClient(Module.client);
  }).addEvent("guildMemberUpdate", async (oldMember, newMember) => {
    if (newMember.guild.id == Module.config.snowflakes.guilds.PrimaryServer) {
  ...
  //    //  let myself = await db.User.get(Module.client.user.id, Module.client.guilds.cache.get(Module.config.snowflakes.guilds.PrimaryServer).id);
