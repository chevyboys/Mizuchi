...
let thankProcess = async (interaction) => {
  ...
  //If there is not a folder in the helpers folder for this guild, create one
  if (!fs.existsSync(`./data/helpers/`)) {
    fs.mkdirSync(`./data/helpers/`);
  }
  if (!fs.existsSync(`./data/helpers/${interaction.guild.id}/`)) {
    fs.mkdirSync(`./data/helpers/${interaction.guild.id}/`);
  }
  ...
  let modCardEmbed = u.embed()
    ...
    .setDescription(`The <@&${Module.config.snowflakes.roles.Helper}> role was given to ${member.displayName} for ${days} day(s), and they were given ${numberOfPointsToGrant} ${validCurrencies.find(c => c.is_primary).name}. \n\n`)
...
let endThank = async () => {
  ...
  for (const guild of Module.client.guilds.cache.values()) {
    ...
    for (let i = 0; i < files.length; i++) {
      let data = JSON.parse(fs.readFileSync(`./data/helpers/${guild.id}/${files[i]}`));
    ...
    for (const oldThank of oldThanks) {
      ...
      fs.unlinkSync(`./data/helpers/${guild.id}/${member.id}.json`);
    }
  }
}
