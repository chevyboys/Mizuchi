...
const loadRoleData = async (Module) => {
  try {
    //for a test, write the roll data to a json file
    rollData = await gs(Module.config.snowflakes.sheets.rolls, true);
...
let rollProcess = async (interaction, hiddenResponse = false) => {
  ...
  if (subcommand === "dice") {
    ...
    if (!diceString || diceString == "" || diceString.toLowerCase() == "help") {
      ...
      if (diceString.toLowerCase().indexOf("prime") > -1) {
        await getRandomDominion("prime", interaction);
      } else if (diceString.toLowerCase().indexOf("deep") > -1) {
        await getRandomDominion("deep", interaction);
      } else if (diceString.toLowerCase().indexOf("dominion") > -1) {
        await getRandomDominion("dominion", interaction);
      } else {
        try {
  ...
  if (!rollData) await loadRoleData(Module);
