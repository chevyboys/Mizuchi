...
class CommandDeniedListener extends Listener {
  ...
  run(error, { interaction }) {
    if (error.context?.silent) {
      U.errorHandler(`${interaction.member.username} (${interaction.member.id}) tried to use a hidden context menu command in ${interaction.channel?.guild ? interaction.channel.name + "; \n" : ""}\`\`\`${interaction.content}\`\`\` `)
