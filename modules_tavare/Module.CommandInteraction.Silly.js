  ...
  .addCommand({
    ...
    customId: `dontpushthebutton`, process: (interaction) => {
      ...
      if (numberOfTimesPressed == 150) {
        ...
        interaction.guild.channels.cache.get(Module.config.snowflakes.channels.botSpam).send(`<@${interaction.user.id}> won the game. Current leaderboard: \n\n${top30.map((value, key) => `<@${key}>: ${value}`).join("\n")}`);
      }
      let response = responses[Math.floor(Math.random() * responses.length)];
      interaction.guild.channels.cache.get(Module.config.snowflakes.channels.secret).send(`<@${interaction.user.id}> pressed the button ${numberOfTimesPressed} times \n\nresponse:\n\`\`\`${response}\`\`\``);
      interaction.reply({
        content: response + "\n\n\n You've pressed the button " + numberOfTimesPressed + " times",
        ephemeral: true
      })
    }
  });
