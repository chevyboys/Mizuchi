...
async function celebrate() {
  let guild = await Module.client.guilds.cache.get(Module.config.snowflakes.guilds.PrimaryServer)
...
async function testcakeOrJoinDays(guild) {
  ...
  try {
    if (!guild || guild == undefined) guild = await Module.client.guilds.fetch(Module.config.snowflakes.guilds.PrimaryServer)
    ...
    for (const person of people_with_a_cakeday) {
      try {
        ...
        //if (member.roles.cache.has(Module.config.snowflakes.roles.CakeDay)) continue;
    ...
    let embed = u.embed()
      ...
      .setColor(guild.roles.cache.get(Module.config.snowflakes.roles.CakeDay).color || guild.members.me.displayHexColor)
    ...
    for (const [years, people] of Object.entries(peopleByYearsInGuild)) {
      ...
      for (const person of people) {
        ...
        //Give them the role
        try {
          await person.member.roles.add(
            Module.config.snowflakes.roles.CakeDay, "Thrice-fold Name Day Celebration");
        } catch (e) {
    ...
    let channel = await guild.channels.fetch(Module.config.snowflakes.channels.general);
    ...
    let cakeDayRole = guild.roles.cache.get(Module.config.snowflakes.roles.CakeDay);
    ...
    for (const [memberId, member] of membersWithRole) {
      if (!ignoreMemberIds.includes(memberId)) {
        try {
          await member.roles.remove(Module.config.snowflakes.roles.CakeDay, "Thrice-fold Name Day Celebration Over");
        } catch (e) {
  ...
  .setClockwork(() => {
    ...
    process: async (interaction) => {
      ...
      if (interaction.options.getBoolean("opt-out") || cakeOrJoinDayDate == "opt-out") {
        ...
        if (target && (interaction.member.roles.cache.has(Module.config.snowflakes.roles.Admin) || interaction.member.roles.cache.has(Module.config.snowflakes.roles.BotMaster) || interaction.member.roles.cache.has(Module.config.snowflakes.roles.Moderator) || interaction.member.roles.cache.has(Module.config.snowflakes.roles.CommunityGuide))) {
        ...
        try {
          ...
          if (bd == 'Invalid Date') {
            ...
            let cakeOrJoinDayUpdateTarget = interaction.member.id
            if (target &&
              (interaction.member.roles.cache.has(Module.config.snowflakes.roles.Admin)
                ...
                || interaction.member.roles.cache.has(Module.config.snowflakes.roles.CommunityGuide))) {
              cakeOrJoinDayUpdateTarget = target.id
            } else if (target && target != interaction.member) {
              return interaction.reply({ content: "You don't have permission to do that", ephemeral: true });
            }
