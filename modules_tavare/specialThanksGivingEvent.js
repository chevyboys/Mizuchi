...
async function setHolidayRole(snowflakes) {
...
async function unsetHolidayRole(snowflakes) {
  const guild = await RoleClient.guilds.fetch(snowflakes.guilds.PrimaryServer);
  const holidayRole = await guild.roles.fetch(snowflakes.roles.Holiday[0]);
...
let thankProcess = async (Module, interaction, bypassWait) => {
  ...
  let modCardEmbed = u.embed()
    .setColor(interaction.guild.roles.cache.get(Module.config.snowflakes.roles.Holiday[0]).hexColor)
    .setAuthor({ iconURL: member.displayAvatarURL(), name: "Happy Thanksgiving!" })
    .setDescription(`Happy Thanksgiving ${member}!\n${interaction.member.displayName} sent you a big thankyou, and The <@&${Module.config.snowflakes.roles.Holiday[0]}> role was given to you for the next hour.`)
    .addField("Reason:", "```" + reason + "```")
  interaction.guild.channels.cache.get(Module.config.snowflakes.channels.botSpam).send({ content: `<@${member.id}>`, embeds: [modCardEmbed], allowedMentions: { parse: ["users"] }, })
  if (!interaction.member.roles.cache.has(Module.config.snowflakes.roles.Admin) && !interaction.member.roles.cache.has(Module.config.snowflakes.roles.Moderator) && !interaction.member.roles.cache.has(Module.config.snowflakes.roles.BotMaster)) {
...
let manageCommand = async (Module) => {
  ...
  if (!thanksgiving() && !debug) {
    if (command) {
      await unsetHolidayRole(Module.config.snowflakes);
      u.get_log_webhook(Module, Module.config.identifier).send({ embeds: [u.embed().setColor("WHITE").setDescription("Thanksgiving event ended")] });
      return commands.delete(command)
    }
  } else if (!command) {
    u.get_log_webhook(Module, Module.config.identifier).send({ embeds: [u.embed().setColor("WHITE").setDescription("Thanksgiving event started")] });
    ...
    await setHolidayRole(Module.config.snowflakes)
    return registeredCommand;
  }
  return null;
}

const kickoff = (Module) => {
  ...
  guild.channels.cache.get(Module.config.snowflakes.channels.general).send({
    embeds: [
      u.embed({
        color: "#f5a442"
        ...
        `\nBe thankful in <#${Module.config.snowflakes.channels.general}> for big things and small` +
...
Module.setClockwork(() => {
  ...
  await manageCommand(Module);
})
  .setClockwork(() => {
    try {
      return setInterval(() => endTurkey(Module), 60 * 60 * 1000);
