...
const canModerateQuestions = interaction => interaction.member.roles.cache.hasAny(
  ...
  Module.config.snowflakes.roles.Admin,
  Module.config.snowflakes.roles.WorldMaker
) || (interaction.guild.channels.cache.get(Module.config.snowflakes.channels.ask)).permissionsFor(interaction.member).any([
  "ADMINISTRATOR",
  "MANAGE_MESSAGES",
  "MODERATE_MEMBERS"
])
    ...
    new MessageActionRow()
      .addComponents(
        ...
        new MessageButton()
          ...
          .setEmoji(Module.config.snowflakes.emoji.upDawn),
        ...
        new MessageButton()
          ...
          .setEmoji(Module.config.snowflakes.emoji.unDawn),
...
async function deleteMessage(messageId, channel) {
  ...
  let c = await Module.client.guilds.cache.get(Module.config.snowflakes.guilds.PrimaryServer).channels.fetch(chan);
  try {
...
async function modAlertCard(interaction, title, message, color) {
  ...
  let card = await interaction.guild.channels.cache.get(Module.config.snowflakes.channels.modRequests).send({ content: content, embeds: [embed], allowedMentions: { roles: [Module.config.snowflakes.roles.Moderator] } });
  return card;
}
...
async function moveToQuestionDiscussion(interaction, targetId) {
  ...
  interaction.reply({ content: `I have moved ${targetQuestion.questionText} to <#${Module.config.snowflakes.channels.questionDiscussion}>`, ephemeral: true });
  let newResponseChannel = interaction.guild.channels.cache.get(Module.config.snowflakes.channels.questionDiscussion)
...
async function restoreToQueue(interaction, targetId, resetVotes) {
  ...
  if (!target) return;
  interaction.reply({ content: `I have moved ${target.questionText} to <#${Module.config.snowflakes.channels.ask}>`, ephemeral: true });
  let newResponseChannel = interaction.guild.channels.cache.get(Module.config.snowflakes.channels.ask);
...
async function ask(interaction, bypassWait) {
  ...
  if (askedRecently.has(interaction.user.id) && !bypassWait) {
    ...
    let idsOfWorldmakers = interaction.guild.roles.cache.get(Module.config.snowflakes.roles.WorldMaker).members.map(m => m.id);
    let interaction_containsWorldmaker = interaction.options.get("question").value.includes("<@&" + Module.config.snowflakes.roles.WorldMaker) || idsOfWorldmakers.some(id => interaction.options.get("question").value.includes("<@" + id)) || idsOfWorldmakers.some(id => interaction.options.get("question").value.includes("<@!" + id));
    ...
    let msg = await interaction.guild.channels.cache.get(Module.config.snowflakes.channels.ask).send({ embeds: [questionQueueEmbed(interaction)], components: row });
...
async function processTransfer(interaction, forceRequestedAnswerer) {
  ...
  for (let i = 0; i < numberOfQuestions && i < sorted.size; i++) {
    ...
    if (sorted.at(i)) {
      ...
      await deleteMessage(sorted.at(i), Module.config.snowflakes.channels.ask);
