  ...
  onAdd: async (reaction, user, Participants) => {
    {
      ...
      let channel = message.guild.channels.cache.get(message.guild.client.config.snowflakes.channels.botSpam);
      let member = await message.guild.members.fetch(user.id);
      if (event.emoji.indexOf(reaction.emoji.toString().toLowerCase()) > -1 && !user.bot && reaction.users.cache.has(message.client.user.id) && message.channel.permissionsFor(message.client.user).has("MANAGE_MESSAGES")) {
          ...
          switch (currentHostileCount) {
            case 5:
              NPCSend(channel,
                u.embed(
                  {
                    description: `<@${user.id}> has achieved the title of <@&${message.guild.client.config.snowflakes.roles.Holiday[0]}>`,
                    footer: {
                      text: `Found Ghosts today: ${Participants.totalHostileToday(user.id)} | total: ${Participants.totalHostile(user.id)}\nFound Spirits  today: ${Participants.totalFriendlyToday(user.id)} | total: ${Participants.totalFriendly(user.id)}`
                    }
                  }
              ...
              await member.roles.add(message.guild.client.config.snowflakes.roles.Holiday[0]);
              message.client.guilds.cache.get(message.guild.client.config.snowflakes.guilds.PrimaryServer).channels.cache.get(event.channel).send({ content: "Welcome to the hidden event channel <@" + user.id + ">!", allowedMentions: { users: [user.id] } });
              break;
            case 50:
              Participants.get(user.id).status = "SUSPENDED";
              Participants.write();
              ...
              await member.roles.add(message.guild.client.config.snowflakes.roles.Holiday[1]);
        ...
        member.roles.cache.hasAny([message.guild.client.config.snowflakes.roles.Admin,
        message.guild.client.config.snowflakes.roles.Helper, message.guild.client.config.snowflakes.roles.Moderator,
        message.guild.client.config.snowflakes.roles.CommunityGuide,
        message.guild.client.config.snowflakes.roles.BotMaster,
        message.guild.client.config.snowflakes.roles.WorldMaker]
          || event.isAdmin(member))) {
        reaction.remove()
