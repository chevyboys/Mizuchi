 ...
 */
  generateRoles: async (guild) => {
    ...
    let holidayRoles = guild.client.config.snowflakes.roles.Holiday.sort((a, b) => guild.roles.cache.get(a).position - guild.roles.cache.get(b).position);
  ...
  cleanRoles: async (guild) => {
    ...
    promises.push(guild.members.fetch().then(m => {
      let promisesRoles = [];
      for (const role of guild.client.config.snowflakes.roles.Holiday) {
        promisesRoles.push(guild.roles.fetch(role).then(
          ...
          */
          r =>
            r.edit({
              name: "Holiday Role " + guild.client.config.snowflakes.roles.Holiday.indexOf(role) + 1,
              color: "#000000",
              hoist: false,
              icon: null
            }).then(discordRole => {
    ...
    return await NPCSend(channel,
      {
        ...
        "fields": [
          {
            "name": "Description:",
            "value": "This is a server event to celebrate both in world and out of world holidays. We have these periodically. The event can be interacted with by finding reactions to messages, or by using the event command (Which will become available once certain conditions are met). Send messages to see where the reactions pop up. You may receive various direct messages and bonus XP roles if you participate in the event to help you on your climb to Emerald\n\nAs a note, spam filters will penalize the scores people who send a lot of low effort posts. Make sure you're on topic for the channels you are in. Server rules will still apply.\n\nThe Book Channels and General have been warded against incursion and will be immune to certain parts of the event. Be aware that this event will change and grow based on how the server interacts with it.",
          },
          {
            "name": "How to opt out:",
      ...
      content: `<@&${guild.client.config.snowflakes.roles.Updates.AllUpdates}>, <@&${guild.client.config.snowflakes.roles.Updates.MetaUpdates}>, <@&${guild.client.config.snowflakes.roles.Updates.HolidayUpdates}>`,
      allowedMentions: { roles: [guild.client.config.snowflakes.roles.Updates.AllUpdates, guild.client.config.snowflakes.roles.Updates.MetaUpdates, guild.client.config.snowflakes.roles.Updates.HolidayUpdates] },
      components: [
        {
          "type": 1,
          "components": [
  ...
  isAdmin: (member) => {
    ...
    return member.permissions.has("ADMINISTRATOR")
      || member.roles.cache.has(member.guild.client.config.snowflakes.roles.Moderator)
      || member.roles.cache.has(member.guild.client.config.snowflakes.roles.Admin)
      || member.roles.cache.has(member.guild.client.config.snowflakes.roles.BotAssistant)
      || member.roles.cache.has(member.guild.client.config.snowflakes.roles.BotMaster);
  },
}
