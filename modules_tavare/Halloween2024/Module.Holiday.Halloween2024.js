...
const rolesClient = create_rolesClient(Module.config);
const rolesClientPrimaryGuild = rolesClient.guilds.fetch(Module.config.snowflakes.guilds.PrimaryServer);
...
async function end(guild) {
  ...
  // For each role in Module.config.snowflakes.roles.Holiday, remove the role from all members
...
async function dailyReset(guild) {
  ...
  await u.get_log_webhook(guild, Module.config.identifier).send({ embeds: [u.embed({ title: "Daily Reset", description: "The event has been reset for the day." })] });
...
Module.addEvent("messageReactionAdd", async (reaction, user) => {
  ...
  if (interaction.customId == "signUpForHolidayUpdates") {
    ...
    await member.roles.add(Module.config.snowflakes.roles.Updates.HolidayUpdates);
