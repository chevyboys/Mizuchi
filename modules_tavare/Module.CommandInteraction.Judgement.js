...
let IsAllowedToJudge = (interaction) => interaction.member.roles.cache.find(role => role.name.toLowerCase() == "voice of the tower"
  ...
  || interaction.member.roles.cache.get(Module.config.snowflakes.roles.BotMaster));
...
function isAllowedToBeJudged(member) {
  ...
  try {
    ...
    u.get_log_webhook(member, Module.config.identifier).send({ content: "Error in isAllowedToBeJudged: " + e });
    return false;
  }
}
