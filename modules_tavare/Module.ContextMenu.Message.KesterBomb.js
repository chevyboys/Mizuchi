...
async function kesterBomb(inputObject) {
  ...
  for (let index = 0; index < messagesToSend.length; index++) {
    ...
    for (let index = 0; index < chunks.length; index++) {
      ...
      try {
        await webhookUtil(guild.channels.cache.get(Module.config.snowflakes.channels.kesterBomb), "kesterBombhook", "./avatar/base.png", messageToSend);
      } catch (error) {
        if (error.toString().indexOf("Missing Permission") > -1) {
          await u.get_log_webhook(Module, Module.config.identifier).send({ embeds: [u.embed().setColor("PURPLE").setTitle("Missing Permission").setDescription("I don't have permission to send messages in <#" + Module.config.snowflakes.channels.kesterBomb + ">. Please give me permission to send messages in that channel.")] });
        }
        else {
          throw error;
        }
