...
class Participant {
  ...
  async #_rewards(client) {
    let guild = client.guilds.cache.get(client.config.snowflakes.guilds.PrimaryServer);
    let member = guild.members.cache.get(this.user);
    let channel = guild.channels.cache.get(client.config.snowflakes.channels.botSpam);
    ...
    switch (this.adjustedCount) {
      ...
      case 500: if (!event.roles[1].role) {
        ...
        try {
          client.guilds.cache.get(client.config.snowflakes.guilds.PrimaryServer).channels.cache.get(event.channel).send("Welcome to the hidden event channel <@" + this.user + ">!");
        } catch (error) {
        }
        break;
    }
