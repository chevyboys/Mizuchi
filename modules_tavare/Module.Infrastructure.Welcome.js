...
function welcomeEscapeSequencesParse(string, member) {
  let parsed = string.replaceAll("[name]", "[honorific] " + member.displayName)
    ...
    .replaceAll("[@Moderator]", `<@&${Module.config.snowflakes.roles.Moderator}>`)
    .replaceAll("[@CommunityGuide]", `<@&${Module.config.snowflakes.roles.CommunityGuide}>`)
    .replaceAll("[@Admin]", `<@&${Module.config.snowflakes.roles.Admin}>`)
    .replaceAll("[#roles]", `<#${Module.config.snowflakes.channels.roles}>`)
    .replaceAll("[#rules]", `<#${Module.config.snowflakes.channels.rules}>`)
    .replaceAll("[#general]", `<#${Module.config.snowflakes.channels.general}>`)
    .replaceAll("[#faq]", `<#${Module.config.snowflakes.channels.faq}>`)
    .replaceAll("[#general]", `<#${Module.config.snowflakes.channels.general}>`)
    .replaceAll("[#spoilerPolicy]", `<#${Module.config.snowflakes.channels.spoilerPolicy}>`)
  return parsed;
}
...
async function generateWelcomeObject(member) {
  const rawWelcome = await gs(Module.config.snowflakes.sheets.welcome);
...
Module
  .addEvent("messageCreate", async (msg) => {
    ...
    try {
      ...
      if (member.guild.id != Module.config.snowflakes.guilds.PrimaryServer) return;


      //set up common variables we will need in a bit
      let guild = member.guild;
      let user = await db.User.get(member.id, member.guild.id);
      let introductions = guild.channels.cache.get(Module.config.snowflakes.channels.introductions); // #introductions
      ...
      if (user) { // Member is returning
        ...
        let toAdd = storedGuildRoles.filter(role => (
          ...
          ![Module.config.snowflakes.guilds.PrimaryServer].includes(role) &&
