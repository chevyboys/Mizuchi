...
async function updateCharacterInfo(Module) {
  characterInfo = await gs(Module.config.snowflakes.sheets.characterInfo);
}


Module.setInit(async () => {
  updateCharacterInfo(Module);
  ...
  () => {
    try {
      return setInterval(() => updateCharacterInfo(Module), 1000 * 60 * 60);
