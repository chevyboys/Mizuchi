...
class DbUserObject {
    ...
    constructor(constructionObj) {
        ...
        if (typeof constructionObj.roles === 'string' || constructionObj.roles instanceof String) {
            ...
            parsedRoles = constructionObj.roles;
     ...
     * @returns {Object} the result of the database update
     */
    updateRoles = async (guildSnowflake) => {
...
let DataBaseActions = {
    User: {
        ...
        */
         ...
         */
        new: async (userID, serverId) => {
            if (!client)
                throw new Error("cannot add user " + userID);
